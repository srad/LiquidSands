...
public class TradeIcon extends Node implements Clickable {
	/**
	 * The trade icon is being displayed over a selected unit. It triggers the
	 * trade of the unit when beeing clicked or indicates the state of trade.
	 * 
	 * @throws Exception
	 */
	public TradeIcon() throws Exception {
		Spatial marker = SpatialFactory.create(SpatialTypes.MARKER_TRADE);
		marker.setLocalScale(0.25f);
	...
	public void onClick(String name) throws IOException, StatusError,
			ProtocolError {
		Game.getInstance().setSelectionMode(SelectionMode.TRADE);
